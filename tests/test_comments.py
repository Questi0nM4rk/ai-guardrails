...
def test_bot_aliases_contains_expected_bots() -> None:
    """BOT_ALIASES includes coderabbit and guardrails-review."""
    assert "coderabbit" in BOT_ALIASES
    assert "guardrails-review" in BOT_ALIASES
    assert BOT_ALIASES["guardrails-review"] == "guardrails-review"
    assert "pr-agent" not in BOT_ALIASES
    assert "claude" not in BOT_ALIASES
...
class TestHelpers:
    ...
    def test_short_bot_name_known(self) -> None:
        """Registered bot aliases: coderabbit and guardrails-review."""
        assert _short_bot_name("coderabbitai") == "coderabbit"
        assert _short_bot_name("guardrails-review") == "guardrails-review"
...
class TestResolveBotName:
    ...
    def test_short_alias_to_login(self) -> None:
        """Registered aliases: coderabbit and guardrails-review."""
        assert _resolve_bot_name("coderabbit") == "coderabbitai"
        assert _resolve_bot_name("guardrails-review") == "guardrails-review"
...
class TestDetectGuardrailsReview:
    """Test HTML-marker-based guardrails-review comment detection."""

    def test_detects_guardrails_review_marker(self) -> None:
        """Comment body containing <!-- guardrails-review --> marker is detected."""
        body = "<!-- guardrails-review -->\n## Review\nThis code has issues."
        assert _detect_guardrails_review(body) is True

    def test_detects_marker_anywhere_in_body(self) -> None:
        """Marker can appear anywhere in the comment body."""
        body = "Some text\n<!-- guardrails-review -->\nMore text"
        assert _detect_guardrails_review(body) is True

    def test_rejects_unrelated_comment(self) -> None:
        """Comments without the marker are not detected."""
        assert _detect_guardrails_review("Deployment successful to staging.") is False

    def test_rejects_empty_body(self) -> None:
        """Empty body is not detected."""
        assert _detect_guardrails_review("") is False

    def test_rejects_partial_marker(self) -> None:
        """Partial marker text is not detected."""
        assert _detect_guardrails_review("guardrails-review") is False


class TestParseThreadGuardrailsReview:
    """Test parse_thread with guardrails-review content-based detection."""

    def test_github_actions_with_guardrails_review_marker_detected(self) -> None:
        """github-actions[bot] with guardrails-review marker sets bot to 'guardrails-review'."""
        node = _make_graphql_node(
        ...
        result = parse_thread(node)
        assert result is not None
        assert result["bot"] == "guardrails-review"

    def test_github_actions_without_marker_unchanged(self) -> None:
        """github-actions[bot] without guardrails-review marker keeps 'github-actions' bot name."""
        node = _make_graphql_node(
            author="github-actions[bot]",
    ...
    def test_guardrails_review_alias_resolves_for_filtering(self) -> None:
        """The 'guardrails-review' alias resolves correctly."""
        assert _resolve_bot_name("guardrails-review") == "guardrails-review"

    def test_filter_by_guardrails_review_matches_detected_threads(self) -> None:
        """Filtering by 'guardrails-review' matches threads with bot='guardrails-review'."""
        threads = [
            _make_thread(bot="guardrails-review", thread_id="PRRT_1"),
            _make_thread(bot="github-actions", thread_id="PRRT_2"),
            _make_thread(bot="coderabbitai", thread_id="PRRT_3"),
        ]
        result = filter_threads(threads, bots=["guardrails-review"], unresolved_only=False)
        assert len(result) == 1
        assert result[0]["bot"] == "guardrails-review"
