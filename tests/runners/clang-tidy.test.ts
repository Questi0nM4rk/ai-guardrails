...
describe("parseClangTidyOutput", () => {
  ...
  test("maps error level to error severity", () => {
    const issues = parseClangTidyOutput(FIXTURE_TEXT);
    const errorIssue = issues.find((i) => i.rule === "clang-tidy/cppcoreguidelines-avoid-assert");
...
describe("clangTidyRunner.generateConfig", () => {
  test("returns a non-null YAML string", () => {
    const output = clangTidyRunner.generateConfig(makeConfig());
    expect(output).not.toBeNull();
    expect(typeof output).toBe("string");
  });

  test("returned string contains Checks key", () => {
    const output = clangTidyRunner.generateConfig(makeConfig());
    expect(output).not.toBeNull();
    if (!output) return;
