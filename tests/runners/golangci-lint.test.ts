...
describe("golangciLintRunner.run", () => {
  test("uses --output.json.path=stdout flag for version >= 1.64", async () => {
    ...
    expect(runner.calls[1]).toEqual(["golangci-lint", "run", "--output.json.path=stdout", "./..."]);
    expect(issues).toHaveLength(2);
  });
  ...
  test("uses --out-format=json for version < 1.64", async () => {
    ...
    expect(runner.calls[1]).toEqual(["golangci-lint", "run", "--out-format=json", "./..."]);
    expect(issues).toHaveLength(2);
  });
});

describe("golangciLintRunner.generateConfig", () => {
  test("returns a non-null YAML string", () => {
    const output = golangciLintRunner.generateConfig(makeConfig());
    expect(output).not.toBeNull();
    expect(typeof output).toBe("string");
  });

  test("returned string contains linters section", () => {
    const output = golangciLintRunner.generateConfig(makeConfig());
    expect(output).not.toBeNull();
    if (!output) return;
