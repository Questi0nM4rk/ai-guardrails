...
def test_empty_threads_and_bodies() -> None:
    """Test handling empty GraphQL response (no threads)."""
    data: dict[str, list[object]] = {"threads": [], "review_bodies": []}
    input_file = io.StringIO(json.dumps(data))
    result = parse_input(input_file)
...
def test_thread_missing_startline_key() -> None:
    """Test thread without optional startLine is handled gracefully."""
    thread = {"path": "test.py", "line": 10, "body": "**Title**"}
    task = parse_thread(thread)
    assert task is not None
...
def test_empty_review_body() -> None:
    """Test empty review body produces no tasks."""
    tasks = parse_review_body("")
    assert tasks == []


def test_review_body_no_sections() -> None:
    """Test review body without nitpick/outside-diff sections."""
    body = "This is just a plain review body with no structured sections."
    tasks = parse_review_body(body)
    assert tasks == []


def test_all_threads_invalid() -> None:
