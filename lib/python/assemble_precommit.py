...
Usage:
    python -m lib.python.assemble_precommit --project-dir /path/to/project
    python -m lib.python.assemble_precommit --languages python go
"""
...
def write_config(config: dict[str, Any], output_path: Path) -> None:
    ...
    with output_path.open("w") as f:
        ...
        f.write("# Generated by: python -m lib.python.assemble_precommit\n")
...
def find_installation_paths() -> tuple[Path, Path]:
    ...
    repo_root = script_dir.parent.parent  # lib/python -> lib -> repo root
...
Examples:
  ...
  python -m lib.python.assemble_precommit
  ...
  python -m lib.python.assemble_precommit --output /tmp/test.yaml
        """,
    )
    parser.add_argument(
        "--project-dir",
