...
Supports CodeRabbit and guardrails-review threads.
...
BOT_ALIASES: dict[str, str] = {
    ...
    "guardrails-review": "guardrails-review",
}

# guardrails-review HTML marker found in comment bodies (posted as github-actions[bot])
_GUARDRAILS_REVIEW_MARKER = "<!-- guardrails-review -->"


def _detect_guardrails_review(body: str) -> bool:
    """Check if a comment body was generated by guardrails-review."""
    return _GUARDRAILS_REVIEW_MARKER in body
...
def parse_thread(node: dict) -> dict | None:
    ...
    # so check body for guardrails-review marker to disambiguate.
    if bot == "github-actions" and _detect_guardrails_review(raw_body):
        bot = "guardrails-review"
...
def run_comments(
    *,
    ...
    if pr is None:
        ...
        if pr is None:
            print(
                "Error: No PR found for current branch. Use --pr NUMBER",
                file=sys.stderr,
            )
            return 1
