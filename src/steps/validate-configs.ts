...
async function validateOne(
  ...
  // Skip if:
  // - File has a valid hash (merged with user settings, hash re-applied)
  // - File has NO hash header at all (user-owned file, not generated by us)
  ...
  const hasHeader = hasHashHeader(content);
  const hashValid = hasHeader && hasValidHash(content);
  const userOwnedFile = !hasHeader;
  if (config !== null && !hashValid && !userOwnedFile) {
