...
export const COMMAND_RULES: CommandRule[] = [
  ...
  callRule("git", { flags: ["-f"], reason: "git push -f" }),
  callRule("git", { flags: ["--hard"], reason: "git reset --hard" }),
  callRule("git", {
    hasDdash: true,
    reason: "git checkout/restore -- (discard working tree changes)",
  }),
  callRule("git", { flags: ["-f"], reason: "git clean -f" }),
  callRule("git", { flags: ["--force"], reason: "git clean --force" }),
  callRule("git", {
    flags: ["--no-verify"],
    reason: "git commit --no-verify (bypasses hooks)",
  }),
  callRule("git", { flags: ["-n"], reason: "git commit -n (bypasses hooks)" }),
  callRule("git", { flags: ["-D"], reason: "git branch -D (force delete)" }),
  callRule("chmod", {
    flags: ["-R"],
